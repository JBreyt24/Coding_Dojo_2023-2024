...
# Create User
    ...
    def validate_user(user_form_data):
        ...
        WHERE email = %(email)s;
        """
        results = connectToMySQL("recipes_schema").query_db(query,user_form_data)
        ...
        if len(user_form_data['first_name']) < 2:
            flash('First name must be as least 2 characters long', "register")
            is_valid = False
        ...
        if len(user_form_data['last_name']) < 2:
            flash('Last name must be as least 2 characters long', "register")
            is_valid = False
        if not EMAIL_REGEX.match(user_form_data['email']):
            flash("Invalid email/password. Please try again", "register")
            is_valid = False
